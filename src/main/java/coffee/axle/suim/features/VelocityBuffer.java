 ...
 * BufferVelocity
 */
public class VelocityBuffer implements Feature {
    ...
    public boolean initialize() {
        try {
            ...
            hook.injectModule(moduleInstance, VelocityBuffer.class);
            renderTimerProperty = hook.createBooleanProperty("timer", false);
            hook.registerProperties(moduleInstance, renderTimerProperty);
    ...
    public void onReceivePacket(ReceivePacketEvent e) {
        if (!hook.isModuleEnabled("BufferVelocity"))
            return;
        if (!Utils.nullCheck()) {
            return;
        }
    ...
    public void onPostMotion(PostMotionEvent e) {
        ...
        if (delaying && ++timeout >= 300) {
            flush();
            hook.sendMessage("&BufferVelocity timed out.");
        }
        s08 = false;
    }
    ...
    public void onRenderTick(TickEvent.RenderTickEvent ev) {
        ...
        try {
            boolean renderTimer = (Boolean) hook.getPropertyValue(renderTimerProperty);
            if (!renderTimer)
                return;
        } catch (Exception e) {
            return;
