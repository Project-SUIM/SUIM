 ...
 * .find <block>
 */
public class FindCommand implements Feature {
    ...
    private void handleCommand(ArrayList<String> args) {
        if (args.size() < 2) {
            hook.sendMessage("Usage: .find <block_name>");
            return;
        }

        if (mc.theWorld == null || mc.thePlayer == null) {
            hook.sendMessage("You must be in a world to use this command!");
            return;
        }

        String blockName = args.get(1).toLowerCase().replace("_", " ");
        hook.sendMessage("Searching for &e" + blockName + "&r...");
        ...
        new Thread(() -> {
            try {
                ...
                mc.addScheduledTask(() -> {
                    if (closestPos == null) {
                        hook.sendMessage(String.format("Could not find any &e%s&r nearby", blockName));
                    } else {
                        ...
                        hook.sendMessage(String.format(
                ...
                mc.addScheduledTask(() -> hook.sendMessage("&cError: " + e.getMessage()));
            }
        }, "BlockFinder").start();
    }
