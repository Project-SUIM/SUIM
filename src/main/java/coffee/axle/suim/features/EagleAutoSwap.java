 ...
 * Eagle auto-swap
 */
public class EagleAutoSwap implements Feature {
    ...
    public boolean initialize() {
        try {
            ...
            eagleModule = hook.findModule("Eagle");
            if (eagleModule == null) {
                MyauLogger.log(getName(), "MODULE_NOT_FOUND");
                return false;
            }

            directionCheckProperty = hook.findProperty(eagleModule, "direction-check");
            pitchCheckProperty = hook.findProperty(eagleModule, "pitch-check");
            blocksOnlyProperty = hook.findProperty(eagleModule, "blocks-only");

            autoSwapProperty = hook.createBooleanProperty("auto-swap", false);

            if (!hook.injectProperty(eagleModule, autoSwapProperty)) {
    ...
    public void onClientTick(TickEvent.ClientTickEvent event) {
        ...
        try {
            ...
            Boolean autoSwap = (Boolean) hook.getPropertyValue(autoSwapProperty);
    ...
    private boolean checkEagleConditions() {
        try {
            if (directionCheckProperty != null) {
                Boolean directionCheck = (Boolean) hook.getPropertyValue(directionCheckProperty);
            ...
            if (pitchCheckProperty != null) {
                Boolean pitchCheck = (Boolean) hook.getPropertyValue(pitchCheckProperty);
            ...
            if (blocksOnlyProperty != null) {
                Boolean blocksOnly = (Boolean) hook.getPropertyValue(blocksOnlyProperty);
