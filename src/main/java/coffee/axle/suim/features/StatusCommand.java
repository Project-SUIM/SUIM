 ...
 * @maybsomeday
 */
public class StatusCommand implements Feature {
    ...
    private void handleCommand(ArrayList<String> args) {
        if (args.size() < 2) {
            hook.sendMessage("§c§lUsage: §7.status §5<playername>");
            return;
        }
        ...
        new Thread(() -> {
            try {
                ...
                if (playerInfo == null) {
                    hook.sendMessage("§c" + username + " not found or API error.");
                    return;
                }
                ...
                if (statusJson == null) {
                    hook.sendMessage("§cFailed to fetch status for " + formattedName);
                    return;
                }
                ...
                if (!statusResponse.has("success") || !statusResponse.get("success").getAsBoolean()) {
                    hook.sendMessage("§cCould not retrieve status for " + formattedName);
                    return;
                }
                ...
                hook.sendMessage("§cerror fetching data for " + username);
                e.printStackTrace();
            }
        }).start();
    }
