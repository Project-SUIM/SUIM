 ...
 */
public class AutoBlockIn implements Feature {
    ...
    public boolean initialize() {
        try {
            ...
            hook.injectModule(moduleInstance, AutoBlockIn.class);

            rangeProperty = hook.createFloatProperty("range", 4.5f, 3.0f, 6.0f);
            speedProperty = hook.createIntegerProperty("speed", 20, 5, 100);
            placeDelayProperty = hook.createIntegerProperty("place-delay", 50, 0, 200);
            rotationToleranceProperty = hook.createIntegerProperty("rotation-tolerance", 25, 5, 100);
            itemSpoofProperty = hook.createBooleanProperty("item-spoof", true);
            showProgressProperty = hook.createBooleanProperty("show-progress", true);
            moveFixProperty = hook.createEnumProperty("move-fix", 1,
    ...
    private void cacheUpdateEventMethods() {
        try {
            Class<?> updateEventClass = hook.getCachedClass(MyauMappings.CLASS_UPDATE_EVENT);
    ...
    private float getRange() {
        try {
            return ((Number) hook.getPropertyValue(rangeProperty)).floatValue();
        } catch (Exception e) {
            return 4.5f;
        }
    }
    ...
    private float getSpeed() {
        try {
            return ((Number) hook.getPropertyValue(speedProperty)).floatValue();
        } catch (Exception e) {
            return 20f;
        }
    }
    ...
    private int getPlaceDelay() {
        try {
            return ((Number) hook.getPropertyValue(placeDelayProperty)).intValue();
        } catch (Exception e) {
            return 50;
        }
    }
    ...
    private int getRotationTolerance() {
        try {
            return ((Number) hook.getPropertyValue(rotationToleranceProperty)).intValue();
        } catch (Exception e) {
            return 25;
        }
    }
    ...
    private boolean getShowProgress() {
        try {
            return (Boolean) hook.getPropertyValue(showProgressProperty);
        } catch (Exception e) {
            return true;
        }
    }
